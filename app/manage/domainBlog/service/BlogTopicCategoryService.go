...
import (
	...
	"github.com/hongmengzhu/xianfu-blog-go/app/models/blog/modBlogTopicCategory"
...
func init() {
	gs.Provide(new(BlogTopicCategoryService))
}

// BlogTopicCategoryService 分类
// @Description:
...
//	@param ct
//	@return rt
func (c *BlogTopicCategoryService) Create(ctx *gin.Context, ct modBlogTopicCategory.CreateCt) (rt rg.Rs[string]) {
	log.Infof(ctx, log.TagAppDef, "ct=%#v", ct)
	var info entityBlog.BlogTopicCategoryEntity
	...
	parent := &entityBlog.BlogTopicCategoryEntity{}
	r := c.sv
	//判断是否是自动,不是自动
...
//	@param ct
//	@return rt
func (c *BlogTopicCategoryService) Update(ctx *gin.Context, ct modBlogTopicCategory.UpdateCt) (rt rg.Rs[string]) {
	log.Infof(ctx, log.TagAppDef, "ct=%#v", ct)
	var info entityBlog.BlogTopicCategoryEntity
	copier.Copy(&info, &ct)
	if ct.ID < 1 {
	...
	if strPg.IsNotBlank(ct.ParentNo) && nil != childData {
		maps := slicePg.ToMapArray(childData, func(t *entityBlog.BlogTopicCategoryEntity) (string, *entityBlog.BlogTopicCategoryEntity) {
		...
		for _, val := range maps {
			for _, item := range val {
				...
				r.Update(ctx, entityBlog.BlogTopicCategoryEntity{IdLink: item.IdLink,
					NoLink: item.NoLink},
					item.ID)
			}
		}
...
//	@receiver c
//	@param id
func (c *BlogTopicCategoryService) childParentIdLink(maps map[string][]*entityBlog.BlogTopicCategoryEntity, parent *entityBlog.BlogTopicCategoryEntity) {
	key := parent.ParentNo
...
func (c *BlogTopicCategoryService) CacheOverride(ctx *gin.Context) {
	...
	maps := slicePg.ToMapArray(infos, func(t *entityBlog.BlogTopicCategoryEntity) (string, *entityBlog.BlogTopicCategoryEntity) {
		if strPg.IsBlank(t.ParentNo) {
			return constNodePg.ROOT, t
		}
		return t.ParentNo, t
	...
	for _, val := range maps {
		for _, item := range val {
			r.Update(ctx, entityBlog.BlogTopicCategoryEntity{
				IdLink: item.IdLink,
				NoLink: item.NoLink},
				item.ID)
		}
...
//	@receiver c
//	@param id
func (c *BlogTopicCategoryService) Detail(ctx *gin.Context, id int64) (rt rg.Rs[modBlogTopicCategory.Vo]) {
	if id < 1 {
		return rt.ErrorMessage("id错误")
	}
	...
	var info modBlogTopicCategory.Vo
...
//	@receiver c
//	@param ct
func (c *BlogTopicCategoryService) Enable(ctx *gin.Context, ct model.BaseIdsCt[string]) (rt rg.Rs[string]) {
...
//	@receiver c
//	@param ct
func (c *BlogTopicCategoryService) Disable(ctx *gin.Context, ct model.BaseIdsCt[string]) (rt rg.Rs[string]) {
...
//	@receiver c
//	@param ct
func (c *BlogTopicCategoryService) State(ctx *gin.Context, ids []string, state enumStatePg.State) (rt rg.Rs[string]) {
	if len(ids) < 1 {
		return rt.ErrorMessage("id错误")
	}
	r := c.sv
	...
	for _, info := range finds {
		if info.State != state.IndexInt8() {
			r.Update(ctx, entityBlog.BlogTopicCategoryEntity{State: state.IndexInt8()}, info.ID)
		}
	}
	return rt.Ok()
}
...
//	@receiver c
//	@param ct
func (c *BlogTopicCategoryService) StateEnableDisable(ctx *gin.Context, ids []string, state enumStatePg.State) (rt rg.Rs[string]) {
...
//	@receiver c
//	@param ct
func (c *BlogTopicCategoryService) LogicalDeletion(ctx *gin.Context, ids []string) (rt rg.Rs[string]) {
	...
	if c.sv.Config().Data.Delete {
		...
		for _, info := range finds {
			...
			if ok, reverse := enum.ReverseEnableDisable(); ok {
				repository.Update(ctx, entityBlog.BlogTopicCategoryEntity{State: reverse.IndexInt8()}, info.ID)
			}
		}
	}
...
//	@receiver c
//	@param ct
func (c *BlogTopicCategoryService) LogicalRecovery(ctx *gin.Context, ids []string) (rt rg.Rs[string]) {
	log.Infof(ctx, log.TagAppDef, "ct=%+v", ids)
	if len(ids) < 1 {
		return rt.ErrorMessage("id错误")
	}
...
//	@receiver c
//	@param ct
func (c *BlogTopicCategoryService) PhysicalDeletion(ctx *gin.Context, ids []string) (rt rg.Rs[string]) {
	log.Infof(ctx, log.TagAppDef, "ct=%+v", ids)
	if len(ids) < 1 {
		return rt.ErrorMessage("id错误")
	}
...
//	@receiver c
//	@param ct
func (c *BlogTopicCategoryService) SelectNodeAllPublic(ctx *gin.Context, ct modBlogTopicCategory.QueryPublicCt) (rt rg.Rs[[]model.BaseNodeNo]) {
	var query entityBlog.BlogTopicCategoryEntity
	copier.Copy(&query, &ct)
	slice := make([]model.BaseNodeNo, 0)
	rt.Data = slice
...
//	@receiver c
//	@param ct
func (c *BlogTopicCategoryService) ExistName(ctx *gin.Context, ct model.BaseExistWdCt[string]) (rt rg.Rs[string]) {
	if "" == ct.Wd {
...
//	@receiver c
//	@param ct
func (c *BlogTopicCategoryService) ExistNo(ctx *gin.Context, ct model.BaseExistWdCt[string]) (rt rg.Rs[string]) {
	if "" == ct.Wd {
		return rt.ErrorMessage("关键词不能为空")
	}
	id := "0"
