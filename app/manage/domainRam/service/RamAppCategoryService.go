...
func init() {
	gs.Provide(new(RamAppCategoryService))
}
	...
	if strPg.IsNotBlank(ct.ParentNo) && nil != childData {
		...
		for _, val := range maps {
			for _, item := range val {
				...
				r.Update(ctx, entityRam.RamAppCategoryEntity{IdLink: item.IdLink,
					NoLink: item.NoLink},
					item.ID)
			}
		}
...
func (c *RamAppCategoryService) State(ctx *gin.Context, ids []string, state enumStatePg.State) (rt rg.Rs[string]) {
	...
	for _, info := range finds {
		if info.State != state.IndexInt8() {
			r.Update(ctx, entityRam.RamAppCategoryEntity{State: state.IndexInt8()}, info.ID)
		}
	}
	return rt.Ok()
}
	...
	if c.sv.Config().Data.Delete {
		...
		for _, info := range finds {
			...
			if ok, reverse := enum.ReverseEnableDisable(); ok {
				repository.Update(ctx, entityRam.RamAppCategoryEntity{State: reverse.IndexInt8()}, info.ID)
			}
		}
	}
