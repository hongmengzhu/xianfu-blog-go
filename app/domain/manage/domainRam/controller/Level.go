...
func init() {
	gs.Provide(new(LevelController)).Name("ManageLevelController").Export(gs.As[routerPg.RouteRegistrar]())
}

// LevelController 用户级别
// @Description:
...
func (c *LevelController) RegisterRoutes(e *gin.Engine) {
	group := e.Group("/xianfu/manage/ram/level", authPg.GroupManageMiddleware(c.Sp))
