...
import (
	...
	configModel2 "github.com/hongmengzhu/xianfu-blog-go/app/domain/manage/domainBasic/service/configModel"
...
func init() {
	gs.Provide(new(BasicConfigModelService))
}
...
func (c *BasicConfigModelService) State(ctx *gin.Context, ids []string, state enumStatePg.State) (rt rg.Rs[string]) {
	...
	for _, info := range finds {
		if info.State != state.IndexInt8() {
			r.Update(ctx, entityBasic.BasicConfigModelEntity{State: state.IndexInt8()}, info.ID)
		}
	}
	return rt.Ok()
}
	...
	if c.sv.Config().Data.Delete {
		...
		for _, info := range finds {
			...
			if ok, reverse := enum.ReverseEnableDisable(); ok {
				repository.Update(ctx, entityBasic.BasicConfigModelEntity{State: reverse.IndexInt8()}, info.ID)
			}
		}
	}
