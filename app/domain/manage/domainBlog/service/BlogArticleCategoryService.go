	...
	if strPg.IsNotBlank(ct.ParentNo) && nil != childData {
		...
		for _, val := range maps {
			for _, item := range val {
				...
				r.Update(ctx, entityBlog.BlogArticleCategoryEntity{IdLink: item.IdLink,
					NoLink: item.NoLink},
					item.ID)
			}
		}
...
func (c *BlogArticleCategoryService) CacheOverride(ctx *gin.Context) {
	...
	for _, val := range maps {
		for _, item := range val {
			r.Update(ctx, entityBlog.BlogArticleCategoryEntity{
				IdLink: item.IdLink,
				NoLink: item.NoLink},
				item.ID)
		}
...
func (c *BlogArticleCategoryService) State(ctx *gin.Context, ids []string, state enumStatePg.State) (rt rg.Rs[string]) {
	...
	for _, info := range finds {
		if info.State != state.IndexInt8() {
			r.Update(ctx, entityBlog.BlogArticleCategoryEntity{State: state.IndexInt8()}, info.ID)
		}
	}
	return rt.Ok()
}
	...
	if c.sv.Config().Data.Delete {
		...
		for _, info := range finds {
			...
			if ok, reverse := enum.ReverseEnableDisable(); ok {
				repository.Update(ctx, entityBlog.BlogArticleCategoryEntity{State: reverse.IndexInt8()}, info.ID)
			}
		}
	}
...
//	@receiver c
//	@param ct
func (c *BlogArticleCategoryService) Query(ctx *gin.Context, ct modBlogArticleCategory.QueryCt) (rt rg.Rs[pagePg.Paginator[modBlogArticleCategory.Vo]]) {
	log.Infof(ctx, log.TagAppDef, "ct=%+v", ct)
	...
	if page.Total > 0 && page.Data != nil && len(page.Data) > 0 {
		pg := pagePg.NewPaginatorByPageable[modBlogArticleCategory.Vo](page.Pageable)
		//字段赋值
...
//	@receiver c
//	@param ct
func (c *BlogArticleCategoryService) QueryAll(ctx *gin.Context, ct modBlogArticleCategory.QueryPublicCt) (rt rg.Rs[[]modBlogArticleCategory.Vo]) {
	var query entityBlog.BlogArticleCategoryEntity
	copier.Copy(&query, &ct)
	slice := make([]modBlogArticleCategory.Vo, 0)
	rt.Data = slice
...
//	@receiver c
//	@param ct
func (c *BlogArticleCategoryService) QueryPublic(ctx *gin.Context, ct modBlogArticleCategory.QueryCt) (rt rg.Rs[pagePg.Paginator[modBlogArticleCategory.Vo]]) {
	var query entityBlog.BlogArticleCategoryEntity
	copier.Copy(&query, &ct)
	slice := make([]modBlogArticleCategory.Vo, 0)
	rt.Data.Data = slice
...
//	@receiver c
//	@param ct
func (c *BlogArticleCategoryService) SelectNodeAll(ctx *gin.Context, ct modBlogArticleCategory.QueryPublicCt) (rt rg.Rs[[]model.BaseNodeNo]) {
	var query entityBlog.BlogArticleCategoryEntity
	copier.Copy(&query, &ct)
