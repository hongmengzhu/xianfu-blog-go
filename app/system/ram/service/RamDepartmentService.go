...
func (c *RamDepartmentService) Update(ctx *gin.Context, ct modRamDepartment2.CreateUpdateCt) (rt rg.Rs[string]) {
	...
	if strPg.IsNotBlank(ct.ParentNo) {
		...
		if parent.No != find.ParentNo {
			result2 := false
			childData, result2 = r.FindAllByNoLink(ctx, find.No, optionsPg.WithCtx(ctx))
			if result2 {
	...
	if strPg.IsNotBlank(ct.ParentNo) && nil != childData {
		...
		for _, val := range maps {
			for _, item := range val {
				...
				err = r.Update(ctx, entityRam.RamDepartmentEntity{IdLink: item.IdLink, NoLink: item.NoLink}, item.ID)
				if err != nil {
					return rt.ErrorMessage(err.Error())
				}
			}
...
func (c *RamDepartmentService) CacheOverride(ctx *gin.Context) {
	...
	for _, val := range maps {
		for _, item := range val {
			r.Update(ctx, entityRam.RamDepartmentEntity{IdLink: item.IdLink, NoLink: item.NoLink}, item.ID)
		}
	}
	maps = nil
}
...
func (c *RamDepartmentService) State(ctx *gin.Context, ids []string, state enumStatePg.State) (rt rg.Rs[string]) {
	...
	ctxR := c.sv
	finds, b := ctxR.FindAllByIdStringIn(ctx, ids, optionsPg.WithCtx(ctx))
	if !b {
		return rt.ErrorMessage("数据不存在")
	}
	for _, info := range finds {
		if info.State != state.IndexInt8() {
			ctxR.Update(ctx, entityRam.RamDepartmentEntity{State: state.IndexInt8()}, info.ID)
		}
	}
	return rt.Ok()
}
	...
	if c.sv.Config().Data.Delete {
		...
		for _, info := range finds {
			...
			if ok, reverse := enum.ReverseEnableDisable(); ok {
				repository.Update(ctx, entityRam.RamDepartmentEntity{State: reverse.IndexInt8()}, info.ID)
			}
		}
	}
