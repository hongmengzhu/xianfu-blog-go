...
// RamDepartmentEntity 部门
type RamDepartmentEntity struct {
	...
	TypeSys     string     `gorm:"column:type_sys;type:varchar(80);index;default:;comment:部门类型|普通|系统" json:"type_sys" comment:"部门类型|普通|系统" `
	ParentId    string     `gorm:"column:parent_id;type:varchar(80);index;default:;comment:上级" json:"parent_id" comment:"上级" `
	IdLink      string     `gorm:"column:id_link;type:text;comment:上级" json:"id_link" comment:"上级" `
...
// TableName RamDepartmentEntity's table name
func (*RamDepartmentEntity) TableName() string {
	return "ram_department"
}

func (*RamDepartmentEntity) TableComment() string {
	return "部门"
}
